# ArtifactView — Claude instructions
...
- Viewer-first, forensic-aware: fast display and responsive browsing come before deep analysis.
- Evidence-driven and confidence-based: model observation vs interpretation; never present uncertainty as fact.
- Preserve provenance always: keep raw extracted values alongside reconciled/preferred values; never silently drop conflicts.
- Safe handling of originals: never overwrite by default; reconstructed/composited outputs export losslessly.
- Modular by layer (UI / application / domain / infrastructure) and by capability, not by file-type silos.
- See docs/design-principles.md and docs/evidence-model.md for the full rules.
