...
public sealed class TimezoneInferenceAnalyzerTests
{
    ...
    [Fact]
    public void Returns_empty_when_longitude_null()
        => Assert.Empty(TimezoneInferenceAnalyzer.Analyze(null, Local(10), Utc(10)));

    [Fact]
    public void Returns_empty_when_capture_date_null()
        => Assert.Empty(TimezoneInferenceAnalyzer.Analyze(2.3, null, Utc(10)));

    [Fact]
    public void Returns_empty_when_gps_utc_null()
        => Assert.Empty(TimezoneInferenceAnalyzer.Analyze(2.3, Local(10), null));

    [Fact]
    public void Consistent_for_paris_longitude_utc_plus_one()
    {
        ...
        Assert.Contains(results, r => r.Id == "timezone-inference-consistent");
    }

    [Fact]
    public void Consistent_for_new_york_longitude()
    {
    ...
    [Fact]
    public void Consistent_for_sydney_utc_plus_ten()
    {
    ...
    [Fact]
    public void Consistent_for_india_half_hour_offset()
    {
    ...
    [Fact]
    public void Mismatch_when_offset_far_from_longitude()
    {
        ...
        Assert.Contains(results, r => r.Id == "timezone-inference-offset-mismatch");
    }

    [Fact]
    public void Mismatch_has_medium_priority()
    {
        var results = TimezoneInferenceAnalyzer.Analyze(2.3, Local(18), Utc(10));
        var match   = results.First(r => r.Id == "timezone-inference-offset-mismatch");
        Assert.Equal(ReviewPriority.Medium, match.ReviewPriority);
    }

    [Fact]
    public void Mismatch_finding_has_supporting_factors()
    {
        var results = TimezoneInferenceAnalyzer.Analyze(2.3, Local(18), Utc(10));
        var match   = results.First(r => r.Id == "timezone-inference-offset-mismatch");
        Assert.NotEmpty(match.SupportingFactors);
    }

    [Fact]
    public void Consistent_finding_mentions_longitude()
    {
        var results = TimezoneInferenceAnalyzer.Analyze(-74.0, Local(10), Utc(15));
        var match   = results.First(r => r.Id == "timezone-inference-consistent");
        Assert.Contains("-74", match.Observation);
    }

    [Fact]
    public void Consistent_confidence_is_reasonable()
    {
        ...
        Assert.InRange(match.ObservationConfidence.Value, 50, 80);
    }

    [Fact]
    public void Utc_prime_meridian_consistent_when_diff_zero()
    {
